...
class SendApprovalEmails extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'applicants:send-approval-emails
                            {--id= : Send to specific applicant ID}
                            {--all : Send to all approved applicants}
                            {--recent : Send to recently approved (last 24 hours)}';
     ...
     * @var string
     */
    protected $description = 'Send approval emails to applicants';
    ...
    public function handle()
    {
        $query = Applicant::where('status', 'approved')
            ->where(function($q) {
                $q->whereNull('emails->approved')
                    ->orWhere('emails->approved', false);
            });
        if ($this->option('id')) {
        ...
        if ($applicants->isEmpty()) {
            $this->info('No approved applicants found with email addresses.');
            return 0;
        }

        $this->info("Found {$applicants->count()} approved applicants with email addresses.");
        ...
        foreach ($applicants as $applicant) {
            ...
            $applicant->update([
                'emails' => empty($applicant->emails) ? [
                    ...
                    'rejected' => false,
                    'approved' => true,
                ] : array_merge($applicant->emails, ['approved' => true])
            ]);
            $bar->advance();
        }
