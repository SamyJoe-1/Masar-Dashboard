...
`;
    ...
    async function downloadPDF() {
        ...
        try {
            // Get all CV pages from the preview
            const pages = container.querySelectorAll('.cv-page');
            
            if (pages.length === 0) {
                throw new Error('No CV pages found to convert');
            }

            // Use html2canvas to convert each page to image
            ...
            for (let i = 0; i < pages.length; i++) {
                ...
                // Add image to PDF page, scaled to fit A4
    ...
    async function downloadImage() {
        ...
        try {
            // Get the currently active page or first page
            const activePage = container.querySelector('.cv-page.active') || container.querySelector('.cv-page');
            
            if (!activePage) {
                throw new Error('No CV page found to convert');
            }

            // Convert the exact rendered page to canvas with high quality
